...
int vm_is_support_pen_touch() {
	return 0;
}
void vm_graphic_fill_rect(unsigned char* buf_, int x, int y, int w, int h, unsigned short c1, unsigned short c2) {
	unsigned short* buf = (unsigned short*)buf_;
	for (int j = std::max(x, 0); j < std::min(x + w, s_w); ++j)
...
void T2Input::show_current_pressed_key() {
	...
	if (current_key >= VM_KEY_NUM1 && current_key <= VM_KEY_NUM9)
		...
		break;
		case VM_KEY_STAR:
			vm_graphic_fill_rect(scr_buf, 0, s_h - key_h, key_w, key_h, fill_color, fill_color);
			break;
		case VM_KEY_NUM0:
			vm_graphic_fill_rect(scr_buf, key_w, s_h - key_h, key_w, key_h, fill_color, fill_color);
			break;
		case VM_KEY_POUND:
			vm_graphic_fill_rect(scr_buf, key_w * 2, s_h - key_h, key_w, key_h, fill_color, fill_color);
			break;
		}
#endif
}
...
void T2Input::handle_penevt(int event, int x, int y) {
	...
	case VM_PEN_EVENT_TAP:
		...
		if (current_key != 255)
			Main::handle_keyevt(VM_KEY_EVENT_DOWN, current_key);
		break;
	case VM_PEN_EVENT_RELEASE:
	case VM_PEN_EVENT_ABORT:
		if (current_key != 255)
			Main::handle_keyevt(VM_KEY_EVENT_UP, current_key);
		current_key = 255;
		break;
	...
	case VM_PEN_EVENT_LONG_TAP:
		if (current_key != 255)
			Main::handle_keyevt(VM_KEY_EVENT_LONG_PRESS, current_key);
		break;
	case VM_PEN_EVENT_REPEAT:
		if (current_key != 255)
			Main::handle_keyevt(VM_KEY_EVENT_REPEAT, current_key);
		break;
...
void T2Input::draw_xy_char(int x, int y, const char* str) {
	...
	for (int i = 0; i < char_height; ++i) {
		unsigned short* scr_buf = (unsigned short*)this->scr_buf + x + (y + i) * s_w;
...
void T2Input::draw_xy_str(int x, int y, const char* str) {
	...
	for (int k = 0; str[k]; ++k) {
		...
		for (int i = 0; i < char_height; ++i) {
			unsigned short* scr_buf = (unsigned short*)this->scr_buf + x + k * char_width + (y + i) * s_w;
			for (int j = 0; j < char_width; ++j)
				if (((*font_ch) >> j) & 1)
					scr_buf[j] = textcolor;
			++font_ch;
...
void T2Input::draw() {
	...
	if (draw_kb) { // draw screen keyboard
		...
		for (int i = 0; i < 5; ++i)
			vm_graphic_line(scr_buf, 0, s_h - (i + 1) * key_h, s_w, s_h - (i + 1) * key_h, 0xFFFF);

		vm_graphic_line(scr_buf, key_w, keyboard_h, key_w, s_h, 0xFFFF);
		vm_graphic_line(scr_buf, key_w * 2, keyboard_h, key_w * 2, s_h, 0xFFFF);

		vm_graphic_line(scr_buf, 0, keyboard_h + key_h / 2, key_w, keyboard_h + key_h / 2, 0xFFFF);
		vm_graphic_line(scr_buf, key_w * 2, keyboard_h + key_h / 2, s_w, keyboard_h + key_h / 2, 0xFFFF);
		...
		case MAIN:
			...
			draw_xy_str(key_w / 2 - strlen("SET") * char_width / 2, s_h - key_h + (key_h - char_height) / 2, "SET");
			draw_xy_str(key_w * 2 + key_w / 2 - 3 * char_width / 2, s_h - key_h + (key_h - char_height) / 2, imput_modes[(int)cur_input_mode]);
	...
	if (!(time - last_input_time >= 1000 || last_input_time > time)) { //draw input mode
		int y = (input_cursor_y == 0 ? char_height : 0), x = s_w - 3 * char_width;
		draw_xy_str_color(x, y, 0xFFFF, gray_color, imput_modes[(int)cur_input_mode]);
		//vm_graphic_fill_rect(scr_buf, 0, s_h - key_h, key_w, key_h, gray_color, gray_color);
	}
	if (!draw_kb) {
