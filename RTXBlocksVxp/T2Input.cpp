...
void T2Input::handle_penevt(VMINT event, VMINT x, VMINT y){
	...
	switch(event){
		...
		case VM_PEN_EVENT_TAP:
			...
			if(current_key!=255)
				handle_keyevt_s(VM_KEY_EVENT_DOWN, current_key);
			break;
		case VM_PEN_EVENT_RELEASE:
		case VM_PEN_EVENT_ABORT:
			if(current_key!=255)
				handle_keyevt_s(VM_KEY_EVENT_UP, current_key);
			current_key = 255;
			break;
		...
		case VM_PEN_EVENT_LONG_TAP:
			if(current_key!=255)
				handle_keyevt_s(VM_KEY_EVENT_LONG_PRESS, current_key);
			break;
		case VM_PEN_EVENT_REPEAT:
			if(current_key!=255)
				handle_keyevt_s(VM_KEY_EVENT_REPEAT, current_key);
			break;
...
void T2Input::draw(){
	...
	if(!(time - last_input_time >= 1000 || last_input_time > time)){ //draw input mode
		int y = (input_cursor_y==0?char_height:0), x = scr_w - 3*char_width;
	...
	if(!draw_kb){
		...
		switch(state){
			case SECOND_CLICK:
				...
				if(x>scr_w-h)
					x=scr_w-h;
				if(scr_h-(input_cursor_y+1)*char_height>h)
